...
def generate_key(passcode):
    return Fernet(base64.urlsafe_b64encode(hashlib.sha256(passcode.encode()).digest()))
...
def decode_message():
    ...
    height, width, _ = img.shape
    key = generate_key(entered_passcode)

    # Extract the length of the message from the image
    ...
    try:
        ...
        messagebox.showerror("Error", f"Incorrect passcode or corrupted image! ({e})")
